...
ADMIN_IDS = {int(id_) for id_ in os.getenv('ADMIN_IDS', '946790544477151243').split(',') if id_.strip()}
ADMIN_ROLE_ID = int(os.getenv('ADMIN_ROLE_ID', '1437461410886975558'))
WATERMARK = "FakeCloud VPS Service"
WELCOME_MESSAGE = "Welcome To FakeCloud! Get Started With Us!"
...
class Database:
    ...
    def increment_stat(self, key, amount=1):
        ...
        self.cursor.execute(
            'INSERT OR REPLACE INTO usage_stats (key, value) VALUES (?, ?)',
            (key, current + amount)
        )
        self.conn.commit()
    ...
    def get_user_vps_count(self, user_id):
        self.cursor.execute(
            'SELECT COUNT(*) FROM vps_instances WHERE created_by = ?',
            (str(user_id),)
        )
    ...
    def get_user_vps(self, user_id):
        self.cursor.execute(
            'SELECT * FROM vps_instances WHERE created_by = ?',
            (str(user_id),)
        )
        columns = [desc[0] for desc in self.cursor.description]
    ...
    def add_vps(self, vps_data):
        ...
        self.cursor.execute(
            f'INSERT INTO vps_instances ({columns}) VALUES ({placeholders})',
    ...
    def update_vps(self, token, updates):
        ...
        self.cursor.execute(
            f'UPDATE vps_instances SET {set_clause} WHERE token = ?',
            values
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
    ...
    def restore_data(self):
        ...
        try:
            ...
            for key, value in data['usage_stats'].items():
                self.cursor.execute(
                    'INSERT INTO usage_stats (key, value) VALUES (?, ?)',
                    (key, value)
                )
...
class LightplaysBot(commands.Bot):
    ...
    async def setup_hook(self):
        # Create SSL context that bypasses TLS handshake issues
            ...
            if vps['status'] == 'running':
                try:
                    ...
                    if container.status != 'running':
                        container.start()
                    logger.info(f"Reconnected container for VPS {vps['vps_id']}")
                except docker.errors.NotFound:
                    logger.warning(f"Container {vps['container_id']} not found, removing")
                        ...
                        for pattern in MINER_PATTERNS:
                            if pattern in output:
                                ...
                                try:
                                    ...
                                    await owner.send(
                                        f"⚠️ Your VPS {vps['vps_id']} has been suspended due to "
                                        f"detected mining activity. Contact admin to unsuspend."
                                    )
                                except Exception:
                                    pass
                                break
...
async def kill_apt_processes(container_id):
    try:
        ...
        await run_docker_command(
            container_id,
            ["bash", "-c", "rm -f /var/lib/apt/lists/lock /var/cache/apt/archives/lock /var/lib/dpkg/lock*"]
        )
        await asyncio.sleep(2)
        return True
    except Exception as e:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if isinstance(status_msg, discord.Interaction):
                await status_msg.followup.send(
                    f"🔄 Waiting for package manager... (Attempt {attempt + 1}/{max_attempts})",
                    ephemeral=True
                )
            else:
                await status_msg.edit(
                    content=f"🔄 Waiting for package manager... (Attempt {attempt + 1}/{max_attempts})"
                )
    ...
    try:
        ...
        if not use_custom_image:
            ...
            success, output = await run_docker_command(
                container_id, ["apt-get", "install", "-y"] + packages
            )
            if not success:
        ...
        success, output = await run_docker_command(
            container_id,
            ["bash", "-c", f"echo {memory_bytes} > /sys/fs/cgroup/memory.max"]
        )
        if not success:
        ...
        success, output = await run_docker_command(
            container_id,
            ["bash", "-c", f"echo '{WATERMARK}' > /etc/machine-info"]
        )
        if not success:
...
async def on_ready():
    ...
    try:
        await bot.change_presence(
            activity=discord.Activity(type=discord.ActivityType.watching, name="Lightplays VPS")
        )
...
`/edit_vps <vps_id>` - Edit VPS specifications
...
async def list_admins(ctx):
    ...
    if ctx.guild:
        ...
        if admin_role:
            role_admins = [f"{m.name} ({m.id})" for m in admin_role.members]
...
async def create_vps_command(
    ctx, memory: int, cpu: int, disk: int, owner: discord.Member,
    ...
    try:
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    hostname=f"lightplays-{vps_id}", mem_limit=memory_bytes,
                    cpu_period=100000, cpu_quota=int(cpu * 100000),
                    cap_add=["ALL"], network=DOCKER_NETWORK,
                    volumes={f'lightplays-{vps_id}': {'bind': '/data', 'mode': 'rw'}},
            ...
            try:
                ...
                container = bot.docker_client.containers.run(
                    DEFAULT_OS_IMAGE, detach=True, privileged=True,
                    hostname=f"lightplays-{vps_id}", mem_limit=memory_bytes,
                    cpu_period=100000, cpu_quota=int(cpu * 100000),
                    cap_add=["ALL"], command="tail -f /dev/null", tty=True,
                    network=DOCKER_NETWORK,
        ...
        setup_success, ssh_password, _ = await setup_container(
            container.id, status_msg, memory, username, vps_id,
        ...
        vps_data = {
            ...
            "memory": memory, "cpu": cpu, "disk": disk, "username": username,
            ...
            "tmate_session": ssh_session_line, "watermark": WATERMARK,
            "os_image": os_image, "restart_count": 0, "last_restart": None,
            "status": "running", "use_custom_image": use_custom_image
        }
        ...
        try:
            ...
            await status_msg.edit(
                content=f"✅ VPS created for {owner.mention}. Check your DMs for connection details."
            )
        except discord.Forbidden:
            await status_msg.edit(
                content=f"❌ Couldn't send a DM to {owner.mention}. Ask them to enable DMs."
            )
    except Exception as e:
...
async def list_vps(ctx):
    try:
        ...
        for vps in user_vps:
            try:
                bot.docker_client.containers.get(vps["container_id"]) if vps["container_id"] else None
            ...
            embed.add_field(
                ...
                value=(
                    f"Status: {status}\n"
                    f"Memory: {vps.get('memory', 'Unknown')}GB\n"
                    f"CPU: {vps.get('cpu', 'Unknown')} cores\n"
                    f"Disk: {vps.get('disk', 'Unknown')}GB\n"
                    f"Username: {vps.get('username', 'Unknown')}\n"
                    f"OS: {vps.get('os_image', DEFAULT_OS_IMAGE)}\n"
                    f"Created: {vps.get('created_at', 'Unknown')}\n"
    ...
    try:
        ...
        for token, vps in all_vps.items():
            try:
                ...
                owner_name = user.name if user else "Unknown User"
            except Exception:
                owner_name = "Unknown User"
            try:
                c = bot.docker_client.containers.get(vps.get("container_id", "")) if vps.get("container_id") else None
                c_status = c.status if c else "Not Found"
            except Exception:
                c_status = "Not Found"
            embed.add_field(
                ...
                value=(
                    f"Owner: {owner_name}\n"
                    f"Status: {vps.get('status','Unknown').capitalize()} (Container: {c_status})\n"
                    f"Memory: {vps.get('memory','?')}GB  CPU: {vps.get('cpu','?')} cores  Disk: {vps.get('disk','?')}GB\n"
                    f"Username: {vps.get('username','?')}  OS: {vps.get('os_image', DEFAULT_OS_IMAGE)}\n"
                    f"Created: {vps.get('created_at','?')}  Restarts: {vps.get('restart_count',0)}"
                ),
                inline=False
            )
            valid_count += 1
        if valid_count == 0:
            ...
            return
        embed.set_footer(text=f"Total VPS instances: {valid_count}")
        await ctx.send(embed=embed)
    except Exception as e:
...
async def vps_stats(ctx, vps_id: str):
    try:
        ...
        try:
            ...
            stdout, stderr = await mem_proc.communicate()
            ...
            disk_stdout, _ = await disk_proc.communicate()
    ...
    try:
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                ...
                f"echo '{vps['username']}:{new_password}' | chpasswd",
                stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed.add_field(name="VPS Instances", value=f"Total: {len(bot.db.get_all_vps())}", inline=True)
        embed.add_field(
            name="Docker Containers",
            value=f"Total: {len(containers)}\nRunning: {len([c for c in containers if c.status == 'running'])}",
            inline=True
        )
        embed.add_field(name="CPU Usage", value=f"{stats['cpu_usage']}%", inline=True)
        embed.add_field(
            name="Memory Usage",
            value=f"{stats['memory_usage']}% ({stats.get('memory_used',0):.2f}GB / {stats.get('memory_total',0):.2f}GB)",
            inline=True
        )
        embed.add_field(
            name="Disk Usage",
            value=f"{stats['disk_usage']}% ({stats.get('disk_used',0):.2f}GB / {stats.get('disk_total',0):.2f}GB)",
            inline=True
        )
        embed.add_field(
            name="Network",
            value=f"Sent: {stats.get('network_sent',0):.2f}MB\nRecv: {stats.get('network_recv',0):.2f}MB",
            inline=True
        )
        embed.add_field(
            name="Container Limit",
            value=f"{len(containers)}/{bot.db.get_setting('max_containers')}",
            inline=True
        )
        if stats.get('last_updated', 0):
...
async def system_info(ctx):
    ...
    try:
        ...
        for partition in psutil.disk_partitions():
            try:
                ...
                disk_info += (
                    ...
                    f"  Mount: {partition.mountpoint}  FS: {partition.fstype}\n"
                    f"  Total: {usage.total/(1024**3):.2f}GB  Used: {usage.used/(1024**3):.2f}GB  "
        ...
        embed.add_field(name="CPU Info", value=f"```"
            f"System: {uname.system} | Node: {uname.node}\n"
            f"Release: {uname.release} | Machine: {uname.machine}\n"
            f"Physical cores: {psutil.cpu_count(logical=False)} | Total cores: {psutil.cpu_count(logical=True)}\n"
            f"CPU Usage: {psutil.cpu_percent()}%```", inline=False)
        embed.add_field(name="Memory Info", value=f"```"
            f"Total: {svmem.total/(1024**3):.2f}GB\n"
            f"Available: {svmem.available/(1024**3):.2f}GB\n"
            f"Used: {svmem.used/(1024**3):.2f}GB ({svmem.percent}%)```", inline=False)
        embed.add_field(name="Disk Info", value=f"```{disk_info}```", inline=False)
        embed.add_field(name="Network Info", value=f"```"
            f"Sent: {net_io.bytes_sent/(1024**2):.2f}MB\n"
            f"Received: {net_io.bytes_recv/(1024**2):.2f}MB```", inline=False)
        await ctx.send(embed=embed)
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            await ctx.send(
                ...
                f"Username: {vps['username']}\nPassword: ||{vps.get('password', 'Not set')}||",
                ephemeral=True
            )
        except Exception as e:
    ...
    try:
        ...
        try:
            ...
            await ctx.send(
                ...
                f"Note: Detach with Ctrl+P then Ctrl+Q",
                ephemeral=True
            )
        except Exception as e:
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Lightplays Global Usage Statistics", color=discord.Color.blue())  # ✅ FIXED
        embed.add_field(name="Total VPS Created", value=bot.db.get_stat('total_vps_created'), inline=True)
        embed.add_field(name="Total Restarts (stat)", value=bot.db.get_stat('total_restarts'), inline=True)
        embed.add_field(name="Current VPS Instances", value=len(all_vps), inline=True)
        embed.add_field(name="Total Memory Allocated", value=f"{total_memory}GB", inline=True)
        embed.add_field(name="Total CPU Cores", value=total_cpu, inline=True)
        embed.add_field(name="Total Disk Allocated", value=f"{total_disk}GB", inline=True)
        embed.add_field(name="Total Restarts (sum)", value=total_restarts, inline=True)
        await ctx.send(embed=embed)
    except Exception as e:
...
async def migrate_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await status_msg.edit(
            content=f"✅ Snapshot {backup_id} created. Download and import on new host: `{backup_file}`"
        )
    except Exception as e:
...
async def suspend_vps(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            await owner.send(f"⚠️ Your VPS {vps_id} has been suspended by an admin.")
        except Exception:
            pass
    except Exception as e:
...
async def edit_vps(ctx, vps_id: str, memory: Optional[int] = None,
                   cpu: Optional[int] = None, disk: Optional[int] = None):
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to use this command!", ephemeral=True)
        return
    ...
    try:
        ...
        try:
            ...
            new_container = bot.docker_client.containers.run(
                vps['os_image'], detach=True, privileged=True,
                hostname=f"lightplays-{vps_id}", mem_limit=memory_bytes,
                cpu_period=100000, cpu_quota=cpu_quota,
                cap_add=["ALL"], command="tail -f /dev/null", tty=True,
                network=DOCKER_NETWORK,
                volumes={f'lightplays-{vps_id}': {'bind': '/data', 'mode': 'rw'}},
                restart_policy={"Name": "always"}
...
async def reinstall_bot(ctx):
    if ctx.author.id != 1210291131301101618:
        ...
        return
    try:
        await ctx.send("🔄 Reinstalling Lightplays bot...")
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        if token:
            bot.db.remove_vps(token)
        embed = discord.Embed(
            title=f"Lightplays VPS Management - {self.vps_id}",
            color=discord.Color.red()
        )
        embed.add_field(name="Status", value="🔴 Container Not Found", inline=True)
        embed.add_field(name="Note", value="This VPS is no longer available.", inline=False)
        for item in self.children:
            item.disabled = True
        await interaction.message.edit(embed=embed, view=self)
        await interaction.response.send_message(
            "❌ This VPS instance is no longer available.", ephemeral=True
        )
        ...
        try:
            ...
            if vps['status'] == 'suspended':
                await interaction.followup.send(
                    "❌ This VPS is suspended. Contact admin to unsuspend.", ephemeral=True
                )
                return
            if container.status == "running":
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                bot.db.update_vps(token, {'status': 'stopped'})
            embed = discord.Embed(  # ✅ FIXED - was discord.Emembed
                title=f"Lightplays VPS Management - {self.vps_id}",
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(
                                title=f"Lightplays VPS Restarted - {self.vps_id}",
                                color=discord.Color.blue()
                            )
                            embed.add_field(
                                name="New SSH Session", value=f"```{ssh_session_line}```", inline=False
                            )
                            await owner.send(embed=embed)
                        except Exception:
                            pass
            ...
            await interaction.followup.send(
                "✅ Lightplays VPS restarted! New SSH details sent to owner.", ephemeral=True
            )
        except Exception as e:
    ...
    async def reinstall_os(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            try:
                bot.docker_client.containers.get(self.container_id)
            except docker.errors.NotFound:
                await self.handle_missing_container(interaction)
                return
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send(
                "🔄 Reinstalling Lightplays VPS... This may take a few minutes.", ephemeral=True
            )
            memory_bytes = vps['memory'] * 1024 * 1024 * 1024
            try:
                container = bot.docker_client.containers.run(
                    image, detach=True, privileged=True,
                    hostname=f"lightplays-{self.vps_id}", mem_limit=memory_bytes,
                    cpu_period=100000, cpu_quota=int(vps['cpu'] * 100000),
                    cap_add=["ALL"], command="tail -f /dev/null", tty=True,
                    network=DOCKER_NETWORK,
                    volumes={f'lightplays-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}}
                )
            except docker.errors.ImageNotFound:
                await status_msg.edit(content=f"❌ Image {image} not found. Using default.")
                container = bot.docker_client.containers.run(
                    DEFAULT_OS_IMAGE, detach=True, privileged=True,
                    hostname=f"lightplays-{self.vps_id}", mem_limit=memory_bytes,
                    cpu_period=100000, cpu_quota=int(vps['cpu'] * 100000),
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(
                            title=f"Lightplays VPS Reinstalled - {self.vps_id}",
                            color=discord.Color.blue()
                        )
            ...
            try:
                ...
                await self.original_message.edit(
                    embed=embed, view=VPSManagementView(self.vps_id, container.id)
                )
            except Exception as e:
        ...
        try:
            ...
            if new_owner_input.startswith('<@') and new_owner_input.endswith('>'):
                ...
                if not new_owner_input.isdigit():
                    await interaction.response.send_message(
                        "❌ Please provide a valid user ID or @mention", ephemeral=True
                    )
                    return
            ...
            if not vps or vps["created_by"] != str(interaction.user.id):
                await interaction.response.send_message(
                    "❌ VPS not found or you don't have permission to transfer it!", ephemeral=True
                )
                return
            try:
            ...
            try:
                ...
                if bot.db.is_user_banned(new_owner.id):
                    await interaction.response.send_message(
                        f"❌ {new_owner.mention} is banned!", ephemeral=True
                    )
                    return
                if bot.db.get_user_vps_count(new_owner.id) >= bot.db.get_setting('max_vps_per_user'):
                    await interaction.response.send_message(
                        f"❌ {new_owner.mention} already has the maximum number of VPS instances.",
                        ephemeral=True
                    )
                    return
            except Exception:
            ...
            await interaction.response.send_message(
                f"✅ VPS {self.vps_id} transferred from {old_owner_name} to {new_owner.name}!",
                ephemeral=True
            )
            try:
                ...
                await interaction.followup.send(
                    "Note: Could not send DM to the new owner.", ephemeral=True
                )
        except Exception as e:
            logger.error(f"Error in TransferVPSModal: {e}")
            await interaction.response.send_message(
                f"❌ Error transferring VPS: {str(e)}", ephemeral=True
            )
...
async def manage_vps(ctx, vps_id: str):
    try:
        ...
        embed = discord.Embed(
            title=f"Lightplays VPS Management - {vps_id}", color=discord.Color.blue()
        )
        embed.add_field(
            name="Status",
            value=f"{vps['status'].capitalize()} (Container: {container_status})",
            inline=True
        )
        embed.add_field(name="Memory", value=f"{vps['memory']}GB", inline=True)
        embed.add_field(name="CPU", value=f"{vps['cpu']} cores", inline=True)
...
@app_commands.describe(vps_id="ID of the VPS to transfer", new_owner="User to transfer the VPS to")
...
if __name__ == "__main__":
    ...
    async def run_bot():
        """Run bot with SSL bypass for TLS handshake issues"""
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE
        ...
        # Patch the HTTP connector before login
